 ...
 * FileId: oPUQb64pwUQ5ykR2+7daGN8fQ2VxLgj9
 */
...
public class Page<T> implements Serializable {
	...
	private static final int MAX_PAGE_NUMBER_LIMIT = 1000;
	...
	public int getTotalPages() {
		int totalPages =  (int) Math.ceil((double) getTotal() / (double) getPageSize());
	 ...
	 * @return 总页数
	 */
	public int getLimitTotalPages() {
		int totalPages =  (int) Math.ceil((double) getTotal() / (double) getPageSize());
		if(totalPages > MAX_PAGE_NUMBER_LIMIT) {
			totalPages = MAX_PAGE_NUMBER_LIMIT;
		}
		return totalPages;
	}
	/**
